...
let is_equal_to__when_actual_different__then_failed (param1, param2) =
  let label = Printf.sprintf "%s [param1 = %d, param2 = %d]" __FUNCTION__ param1 param2 in
  label >:: fun _ ->
    (* when *)
...
let is_equal_to_Test_list =
  ...
  @@ List.concat
    [ List.map (fun p -> is_equal_to__when_actual_is_same__then_passed p) positive_params;
      List.map (fun p -> is_equal_to__when_actual_different__then_failed p) pair_params ]
...
let not_is_equal_to_Test_list =
  ...
  @@ List.concat
    [ List.map (fun p -> not_is_equal_to__when_actual_is_same__then_failed p) positive_params;
      List.map (fun p -> not_is_equal_to__when_actual_is_different__then_passed p) pair_params ]
...
let is_greater_than_or_equal_to__when_actual_less__then_failed (param1, param2) =
  ...
  label >:: fun _ ->
    ...
    (* then *)
    let expected =
      Assertion_failed (Printf.sprintf "Expected %d to be greater than or equal to %d" param1 param2)
    in
    assert_that action @@ Is.raising expected
...
let is_less_than__when_actual_greater__then_failed (param1, param2) =
  ...
  label >:: fun _ ->
    ...
    (* then *)
    let expected = Assertion_failed (Printf.sprintf "Expected %d to be less than %d" param2 param1) in
    assert_that action @@ Is.raising expected
...
let not_is_less_than_or_equal_to__when_actual_less__then_failed (param1, param2) =
  ...
  label >:: fun _ ->
    ...
    (* then *)
    let expected =
      Assertion_failed (Printf.sprintf "Expected %d not to be less than or equal to %d" param1 param2)
    in
    assert_that action @@ Is.raising expected
